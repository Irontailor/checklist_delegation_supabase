...
export const maintenanceData = createAsyncThunk("maintenanceData", async (arg = 1, { rejectWithValue }) => {
    try {
        ...
        const doerFilter = typeof arg === 'object' ? (arg.doerFilter || '') : '';

        const response = await fetchMaintenanceDataSortByDate(page, limit, searchTerm, frequency, dateFilter, deptFilter, doerFilter);
        return response;
    } catch (error) {
        return rejectWithValue(error.message);
    }
