...
export const uniqueChecklistTaskData = createAsyncThunk(
  'fetch/checklistTask',
  async ({ page = 0, pageSize = 50, nameFilter = '', dateFilter = 'all', append = false, deptFilter = '', doerFilter = '' }) => {
    const result = await fetchChecklistData(page, pageSize, nameFilter, dateFilter, deptFilter, doerFilter);
...
export const uniqueDelegationTaskData = createAsyncThunk(
  ...
  async ({ page = 0, pageSize = 50, nameFilter = '', dateFilter = 'all', append = false, deptFilter = '', doerFilter = '' }) => {
    const result = await fetchDelegationData(page, pageSize, nameFilter, dateFilter, deptFilter, doerFilter);
    return { ...result, append };
  }
);
